...
def Model_CNN(Input_Shape=None, Num_Output=None, LR=None):
    ...
    conv1d = Conv1D(filters=32, kernel_size=3, padding='same', activation='relu')(input_1)
    conv1d = BatchNormalization()(conv1d)
    conv1d = Conv1D(filters=64, kernel_size=3, padding='same', activation='relu')(input_1)
    conv1d = BatchNormalization()(conv1d)
    conv1d = MaxPooling1D(pool_size=2)(conv1d)
    conv1d = BatchNormalization()(conv1d)
    
    conv1d = Conv1D(filters=64, kernel_size=3, padding='same', activation='relu')(conv1d)
    conv1d = BatchNormalization()(conv1d)
    conv1d = Conv1D(filters=128, kernel_size=3, padding='same', activation='relu')(conv1d)
    conv1d = BatchNormalization()(conv1d)
    conv1d = MaxPooling1D(pool_size=2)(conv1d)
    conv1d = BatchNormalization()(conv1d)
    ...
    conv1d = Conv1D(filters=128, kernel_size=3, padding='same', activation='relu')(conv1d)
    conv1d = BatchNormalization()(conv1d)
    maxpooling1d = GlobalMaxPooling1D()(conv1d)
    
    attention_1 = Dense(16, activation="relu")(maxpooling1d)
    attention_1 = Dense(78, activation="softmax")(conv1d)
    attention_1 = BatchNormalization()(attention_1)
    
    attention_1 = Flatten()(attention_1)  # squeeze (None,50,1)->(None,50)
    attention_1 = Activation("softmax")(attention_1)
    
    model = Model(input_1, attention_1)
    opt = RMSprop(lr=LR)
    model.compile(loss="categorical_crossentropy", optimizer=opt , metrics=['accuracy', _F1score, _Precision, _Recall, _Matthews_correlation])
